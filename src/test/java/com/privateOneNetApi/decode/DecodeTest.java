package com.privateOneNetApi.decode;

import com.privateOneNetApi.common.DecodeUtil;
...
public class DecodeTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(DecodeTest.class);
    /**
     * 功能描述：第三方平台数据接收。<p>
     *           <ul>注:
    ...
    public String receive(@RequestBody String body) throws NoSuchPaddingException, InvalidKeyException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
         ...
         **************************************************/
        /*************明文模式  start****************/
        DecodeUtil.BodyObj obj = DecodeUtil.resolveBody(body, false);
        logger.info("data receive:  body Object --- " +obj);
        if (obj != null){
            boolean dataRight = DecodeUtil.checkSignature(obj, token);
            if (dataRight){
         ...
         ********************************************************/
        /*************加密模式  start****************/
//        DecodeUtil.BodyObj obj1 = DecodeUtil.resolveBody(body, true);
//        logger.info("data receive:  body Object--- " +obj1);
//        if (obj1 != null){
//            boolean dataRight1 = DecodeUtil.checkSignature(obj1, token);
//            if (dataRight1){
//                String msg = DecodeUtil.decryptMsg(obj1, aeskey);
...
//        }
    ...
    public String check(@RequestParam(value = "msg") String msg,
        ...
        if (DecodeUtil.checkToken(msg,nonce,signature,token)){
            return msg;
        }else {
            return "error";
        }
